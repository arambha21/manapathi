...
!define PUBLISHER 'Manapathi S.A.'
...
!ifndef SERVICENAME
	!define SERVICENAME 'manapathi-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\manapathibuild'
!endif

!define PRODUCT_NAME "Manapathi"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Manapathi Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\manapathi_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\manapathi-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\manapathi-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\manapathi-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\manapathi-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.manapathi.com/page/contactus"
...
; English
LangString DESC_Manapathi_Server ${LANG_ENGLISH} "Install the Manapathi Server with all the Manapathi standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Manapathi."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Manapathi for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Manapathi Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "Manapathi Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "Manapathi Server Only"
LangString Profile_IOT ${LANG_ENGLISH} "Manapathi IoT"
LangString TITLE_Manapathi_Server ${LANG_ENGLISH} "Manapathi Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString TITLE_IOT ${LANG_ENGLISH} "Manapathi IoT"
LangString TITLE_Nginx ${LANG_ENGLISH} "Nginx WebServer"
LangString TITLE_Ghostscript ${LANG_ENGLISH} "Ghostscript interpreter"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Manapathi"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_Manapathi_Server ${LANG_FRENCH} "Installation du Serveur Manapathi avec tous les modules Manapathi standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Manapathi."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Manapathi pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Manapathi"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur Manapathi Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur Manapathi"
LangString Profile_IOT ${LANG_FRENCH} "Manapathi IoT"
LangString TITLE_Manapathi_Server ${LANG_FRENCH} "Serveur Manapathi"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString TITLE_IOT ${LANG_FRENCH} "Manapathi IoT"
...
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Manapathi"
...
Section $(TITLE_Manapathi_Server) SectionManapathi_Server
    SectionIn 1 2 3
    ...
    # If there is a previous install of the Manapathi Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "addons_path" "$INSTDIR\server\manapathi\addons"
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    # Set data_dir
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "data_dir" "$INSTDIR\sessions"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\manapathi-bin" --stop-after-init -c "$INSTDIR\server\manapathi.conf" --logfile "$INSTDIR\server\manapathi.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\manapathi-bin\" -c "\"$INSTDIR\server\manapathi.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    Call RestartManapathiService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_Manapathi" \
...
Section $(TITLE_IOT) IOT
    ...
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "server_wide_modules" "web,hw_posbox_homepage,hw_drivers"
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\server\manapathi.conf" "options" "max_cron_threads" "0"
    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\server\manapathi-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
SectionEnd
...
Section $(TITLE_Nginx) Nginx
    ...
    File "..\..\manapathi\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\manapathi\addons\point_of_sale\tools\posbox\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@manapathi.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.manapathi.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@manapathi.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionManapathi_Server} $(DESC_Manapathi_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\Manapathi ${VERSION}"
...
Function ShowProxyTokenDialogPage
    ...
    doProxyToken:
        ...
        ${NSD_CreateLabel} 0 0 100% 25% "Here is your access token for the Manapathi IOT, please write it down in a safe place, you will need it to configure the IOT"
        Pop $ProxyTokenLabel
...
Function ComponentLeave
    SectionGetFlags ${SectionManapathi_Server} $0
...
Function RestartManapathiService
    DetailPrint "Restarting Manapathi Service"
