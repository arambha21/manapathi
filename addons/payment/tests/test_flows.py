...
from manapathi.tests import tagged, JsonRpcException
from manapathi.tools import mute_logger

from manapathi.addons.payment.controllers.portal import PaymentPortal
from manapathi.addons.payment.tests.http_common import PaymentHttpCommon
...
class TestFlows(PaymentHttpCommon):
    ...
    def _test_flow(self, flow):
        ...
        with mute_logger('manapathi.addons.payment.models.payment_transaction'):
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger("manapathi.http"), self.assertRaises(
            JsonRpcException,
            msg='manapathi.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.',
        ):
            self._portal_transaction(**transaction_values)

    @mute_logger('manapathi.http')
    def test_transaction_route_rejects_unexpected_kwarg(self):
        route_kwargs = {
            **self._prepare_pay_values(),
            'custom_create_values': 'whatever',  # This should be rejected.
        }
        with self.assertRaises(JsonRpcException, msg='manapathi.exceptions.ValidationError'):
    ...
    def test_transaction_wrong_token(self):
        ...
        with mute_logger('manapathi.http'), self.assertRaises(JsonRpcException, msg='manapathi.exceptions.ValidationError: The access token is invalid.'):
    ...
    @mute_logger('manapathi.addons.payment.models.payment_transaction')
    def test_direct_payment_triggers_no_payment_request(self):
        self.authenticate(self.portal_user.login, self.portal_user.login)
        self.partner = self.portal_partner
        self.user = self.portal_user
        with patch(
            'manapathi.addons.payment.models.payment_transaction.PaymentTransaction'
