...
commandProviderRegistry.add("link dialog", {
    async provide(env, { sessionId }) {
        ...
        if (lastSessionId !== sessionId) {
            ...
            const selection = wysiwyg && wysiwyg.manapathiEditor && wysiwyg.manapathiEditor.document.getSelection();
            const range = selection && selection.rangeCount && selection.getRangeAt(0);
            if (range) {
                label = !wysiwyg.getInSelection('a') ? 'Create link' : 'Edit link';
                action = () => {
                    const selection = wysiwyg.manapathiEditor.document.getSelection();
