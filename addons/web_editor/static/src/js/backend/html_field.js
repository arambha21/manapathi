...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/manapathi-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.manapathiEditor) {
            const selection = this.wysiwyg.manapathiEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("manapathi-editor-qweb");
        ...
        this.wysiwyg.manapathiEditor.addEventListener("historyStep", () =>
            this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
        );

        if (this.props.isCollaborative) {
            this.wysiwyg.manapathiEditor.addEventListener("onExternalHistorySteps", () =>
                this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
            );
        }
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.manapathiEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.manapathiEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.manapathiEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.manapathiEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.manapathiEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent, shouldInline } = {}) {
        ...
        if (this._isDirty() || urgent || (shouldInline && this.props.isInlineStyle)) {
            ...
            if (this.wysiwyg && this.wysiwyg.manapathiEditor) {
                ...
                this.wysiwyg.manapathiEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.manapathiEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await manapathi.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await manapathi.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await ensureJQuery();
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await manapathi.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await manapathi.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.manapathiEditor.sanitize(this.wysiwyg.manapathiEditor.editable);
        const html = this.wysiwyg.getValue();
        const $manapathiEditor = $editable.closest('.manapathi-editor-editable');
        // Save correct nodes references.
        // Remove temporarily the class so that css editing will not be converted.
        $manapathiEditor.removeClass('manapathi-editor-editable');
        $editable.html(html);
        await toInline($editable, { $iframe: this.wysiwyg.$iframe, wysiwyg:this.wysiwyg });
        $manapathiEditor.addClass('manapathi-editor-editable');
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(manapathi.debug && options.codeview),
            placeholder: attrs.placeholder,
