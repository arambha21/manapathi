/** @manapathi-module **/
import { setSelection } from "@web_editor/js/editor/manapathi-editor/src/utils/utils";
...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("typing valid URL + space should convert to link", async function (assert) {
                ...
                const node = editable.querySelector("p");
                setSelection(node, 0);
                inputText(".manapathi-editor-editable p", "http://google.co.in");
                insertText(editor, " ");
                editor.clean();
                assert.strictEqual(
                    editable.innerHTML,
        ...
        QUnit.test("typing invalid URL + space should not convert to link", async function (assert) {
            ...
            const node = editable.querySelector("p");
            setSelection(node, 0);
            inputText(".manapathi-editor-editable p", "www.manapathi");
            insertText(editor, " ");
            editor.clean();
            assert.strictEqual(editable.innerHTML, "<p>www.manapathi </p>");
        });
